...
const Home = () => {
  ...
  const [dataFilter, dispatchDataFilter] = useReducer(
    filteredDataReducer,
    initialFilterData
  );
  ...
  useEffect(() => {
    ...
    const filteredBooks: IBook[] = data.books
      .filter((b) => filterCategory(b, category))
      .filter((b) => filterState(b, state))
      .filter((b) => filterSearch(b, search));
