...
const History = () => {
  ...
  const formatOrders = (orders: IOrders[]) => {
    const newOrders = orders.map((o) => {
      ...
      return {
        ...
        amount: o.amount,
        date: formatDate.toLocaleDateString("en-US"),
        price: `${totalPrice} $`,
      };
    });
  ...
  const onHandleChangeOrder = async (event: ChangeEvent<HTMLSelectElement>) => {
    ...
    const ordersOrderBy = await getOrderByUserIdOrderBy(
  ...
  const table =
    orders.length > 0 ? (
      <Fragment>
        ...
        <Table
          th={["Book", "Amount", "Date", "Price"]}
          td={stateOrders}
          properties={["bookName", "amount", "date", "price"]}
        />
      </Fragment>
    ) : (
...
export async function loader() {
  const _token = await getCookie("token");
  const _orders: IOrders[] = await getOrderByUserId(
    validateToken(_token).id,
    _token
  );
