...
const Cart = () => {
  ...
  const htmlCart = cart.cart.length ? (
    <table>
      <thead>
        <tr>
          <th>Books</th>
          ...
          <tr key={a.bookId}>
            ...
            <td>
              <span className={style.price}>
                {(a.book.price * a.amount).toFixed(2)} $
              </span>
            </td>
            <td className={style.actions}>
              <span
                className={`${style.plus} ${a.amount >= 10 && style.disabled}`}
                onClick={() => increseAumount(a.bookId)}
              ></span>
              <span
                className={`${style.min} ${a.amount <= 1 && style.disabled}`}
                onClick={() => decreseAmount(a.bookId)}
              ></span>
            </td>
            <td className={style.remove}>
              <button
                className={style["remove-btn"]}
                onClick={() => handleClickRemove(a.book.name, a.bookId)}
              >
                Remove
              </button>
            </td>
    ...
    <NoElements text="There is nothing in the shopping cart" />
  );
  ...
  return navigation.state === "loading" ? (
    ...
    <>
      <Container>
        ...
        {htmlCart}
        {cart.cart.length && (
          <button className={style.btn} onClick={() => closeBuy(true)}>
            Buy - {totalPrice.toFixed(2)} $
          </button>
        )}
      </Container>
      {modal.modal && (
        <Modal title={"Remove"} setModal={closeModal}>
          <Confirm
            id={modal.bookId}
            question={modal.title}
            closeModal={closeModal}
            handleClick={removeElementOfCart}
          />
        </Modal>
      )}
      {buy && (
        <Modal title={"Buy"} setModal={closeBuy}>
          {cart.cart.map((a) => (
            <div className={style.details} key={a.bookId}>
              <table>
                <tbody>
                  <tr>
                    <td>x{a.amount}</td>
                    ...
                    <td>
                      <span className={style.details__description}>
                        {a.book.description}
                      </span>
                    </td>
                  </tr>
          ...
          <Direction setModal={closeBuy} totalPrice={totalPrice.toFixed(2)} />
        </Modal>
      )}
    </>
  );
